...
// Recursive helper: builds a tree from a slice of index entries all sharing
// the same directory prefix at the given depth.
//
// entries: pointer to first IndexEntry for this level
// count:   number of entries at/under this level
// depth:   current directory depth (0 = root)
// id_out:  output for the written tree object's hash
static int write_tree_level(const IndexEntry *entries, int count, int depth, ObjectID *id_out) {
    ...
    int i = 0;
    while (i < count) {
        // Find the component at 'depth' in entries[i].path
        const char *path = entries[i].path;

        // Navigate to the component at 'depth'
        const char *p = path;
        ...
        // Check if there's another '/' after this component
        ...
        if (!slash) {
            ...
            // Copy just the filename component
            ...
            // This is a subdirectory — group all entries sharing the same subdir
            ...
            // Find how many entries belong to this subdirectory
            int j = i;
            while (j < count) {
                ...
                size_t this_dir_len = (size_t)(sl - pp);
                if (this_dir_len != dir_len || strncmp(pp, dir_name, dir_len) != 0) break;
                j++;
            }
...
// Helper to sort IndexEntry by path
...
int tree_from_index(ObjectID *id_out) {
    ...
    // Sort entries by path to group subdirectories together

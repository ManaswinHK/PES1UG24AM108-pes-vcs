...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // header_len does NOT include null terminator — we add 1 byte for '\0'

    // 2. Allocate and build the full object: header + '\0' + data
    ...
    // 4. Check for deduplication — if already stored, return early
    if (object_exists(&id)) {
        if (id_out) *id_out = id;
        free(full_obj);
        return 0;
    ...
    // 5. Build the shard directory path and create it
    char hex[HASH_HEX_SIZE + 1];
    hash_to_hex(&id, hex);
    ...
    // 7. Write to a temp file in the shard directory
    char tmp_path[520];
    ...
    // 8. fsync the file to ensure data reaches disk
    fsync(fd);
    close(fd);
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    // 3. Verify integrity: recompute the hash and compare against expected
    ObjectID computed;
    ...
    // 6. Extract size from header and validate
    size_t header_len = (size_t)(null_byte - raw);
    size_t data_len = (size_t)file_size - header_len - 1; // subtract header + '\0'

    // 7. Allocate and copy out the data portion
    uint8_t *data_copy = malloc(data_len + 1); // +1 for convenient null termination
    if (!data_copy) {
        free(raw);
        return -1;
    }
    memcpy(data_copy, null_byte + 1, data_len);
    data_copy[data_len] = '\0'; // null-terminate for string safety
